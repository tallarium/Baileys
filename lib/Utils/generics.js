...
const printQRIfNecessaryListener = (ev, logger) => {
    ev.on('connection.update', ({ qr }) => {
        printQrInTerminal(qr)
            .catch((err) => logger.error(JSON.stringify(err), 'Error when printing QR in terminal'));
    });
};
exports.printQRIfNecessaryListener = printQRIfNecessaryListener;
async function printQrInTerminal(qr) {
    if (qr) {
        const QR = await Promise.resolve().then(() => __importStar(require('qrcode-terminal')));
