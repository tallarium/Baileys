...
const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    const img = (0, sharp_1.default)(bufferOrFilePath);
...
async function getAudioDuration(buffer) {
    ...
    if (Buffer.isBuffer(buffer)) {
        metadata = await music_metadata_1.default.parseBuffer(buffer, undefined, options);
    }
    else if (typeof buffer === 'string') {
        metadata = await music_metadata_1.default.parseFile(buffer, options);
    }
    else {
        metadata = await music_metadata_1.default.parseStream(buffer, undefined, options);
    }
...
async function getAudioWaveform(buffer, logger) {
    try {
        ...
        const audioBuffer = await (0, audio_decode_1.default)(audioData);
