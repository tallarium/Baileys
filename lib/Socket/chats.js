...
const makeChatsSocket = (config) => {
    ...
    const resyncMainAppState = async (ctx) => {
        ...
        await (mutationMutex.mutex(() => resyncAppState(Types_1.ALL_WA_PATCH_NAMES, ctx).catch(err => (onUnexpectedError(err, 'main app sync')))));
    };
    ...
    */
    ...
    async function syncNodeHandler(node) {
    ...
    ws.on('CB:notification,type:server_sync', (node) => {
        ...
        if (update) {
            ...
            mutationMutex.mutex(() => (resyncAppState([name], undefined))).catch(err => onUnexpectedError(err, `sync state from node: ${node}`));
        }
    });
