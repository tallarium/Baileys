...
const makeMessagesSocket = async (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: axiosOptions, patchMessageBeforeSending, cachedGroupMetadata } = config;
    const groupsSock = await (0, groups_1.makeGroupsSocket)(config);
    const sock = (0, newsletter_1.makeNewsletterSocket)(groupsSock);
    const { ev, authState, processingMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupMetadata, groupToggleEphemeral, onUnexpectedError } = sock;
